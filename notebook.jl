...
# ╔═╡ a799738f-c985-47cc-8f12-e2e86cd4b92f
@bind N₁ Slider(1:1:10)
...
# ╔═╡ 255c7a94-3b57-47e2-b0a7-b5b17b5455b2
@bind N₂ Slider(1:1:10)

# ╔═╡ 7af742d9-e32f-4c1c-b4d5-de78414b4269
md"""
N₂ = $(N₂)
"""

# ╔═╡ a3541eaa-29e1-42b1-807f-bab5de3f7986
md"""
Plot:
"""
...
begin
	...
	if "Energy" in plotting
		plot!(plt, ϕ, χN/√(N₁*N₂).*ϕ.*(1 .-ϕ), label=L"\Delta U_\mathrm{mix}", color=myscheme[3])
	end

	if "Free energy" in plotting
		plot!(plt, ϕ,_f, xlims=(0,1), label=L"\Delta F_\mathrm{mix}", color=myscheme[2])
	end
	if χ > 1/2*(1/√(N₁)+1/√(N₂))^2 && ("Binodal" in plotting || "Spinodal" in plotting)
		
		f_obj!(F,ϕ) = begin
			F[1] = df(exp10(ϕ[1]),χ,N)-df((1-exp10(ϕ[2])),χ,N) 
			F[2] = f(exp10(ϕ[1]),χ,N)-f((1-exp10(ϕ[2])),χ,N)-df(exp10(ϕ[1]),χ,N)*exp10(ϕ[1])+df((1-exp10(ϕ[2])),χ,N)*(1-exp10(ϕ[2]))
		end

		p = Polynomial([N₂,(N₁-N₂-2*χ*N₁*N₂),2*χ*N₁*N₂])
		...
		fₛ2 = f(ϕₛ2,χ,[N₁,N₂])
		# g_obj!(F,ϕ) = begin
		# 	F[1] = d2f(exp10(ϕ[1]),χ,N)
		# end

		# sol_g = Clapeyron.Solvers.nlsolve(g_obj!,big.([-0.1]), LineSearch(Newton()),NEqOptions(),ForwardDiff.Chunk{1}())
		# println(Clapeyron.Solvers.x_sol(sol_g))
		
		sol = Clapeyron.Solvers.nlsolve(f_obj!,big.([log10(ϕₛ1/2),log10((1-ϕₛ2)/2)]))

		(ϕᵢ,ϕᵢᵢ) = Clapeyron.Solvers.x_sol(sol)

		fᵢ  = f(exp10(ϕᵢ),χ,[N₁,N₂])
		fᵢᵢ = f(1-exp10(ϕᵢᵢ),χ,[N₁,N₂]) 
		if isnan(fᵢᵢ)
			fᵢᵢ = 0.
		end
		plot!(plt,[exp10(ϕᵢ),1-exp10(ϕᵢᵢ)],[fᵢ,fᵢᵢ],label="", linestyle=:dash
			, color=myscheme[4])
		plot!(plt,[exp10(ϕᵢ),1-exp10(ϕᵢᵢ)],[fᵢ,fᵢᵢ],label="",seriestype=:scatter, markerstrokecolor=myscheme[4], marker=:square, color=:white)

		plot!(plt, [ϕₛ1, ϕₛ2],[fₛ1, fₛ2],label="",seriestype=:scatter, markerstrokecolor=myscheme[2], marker=:utriangle, color=:white)
	end
	plt
end

# ╔═╡ dbba873e-f3aa-4226-9181-9149c31a8ae7
md"""
## Phase diagram
"""

# ╔═╡ 2fe8af57-dae6-41a3-b76b-de484ee4cf0b
@bind N₁p Slider(1:1:100)

# ╔═╡ a3212f1e-a258-4af4-99cf-f71077bcca55
md"""
N₁ = $(N₁p)
"""

# ╔═╡ 5f7d5833-7c90-4425-a9c5-53c5902ffd20
@bind N₂p Slider(1:1:100)

# ╔═╡ 0afea427-182b-4ee1-b5eb-a83df0d50f73
md"""
N₂ = $(N₂p)
"""

# ╔═╡ 9fa62ad5-53dc-4b46-8efd-732bc317798e
begin
	Np = [N₁p,N₂p]
	χc = 1/2*(1/√(N₁p)+1/√(N₂p))^2
	χp = LinRange(χc+1.,χc,400)
	...
	p2 = Polynomial([N₂p,(N₁p-N₂p-2*χp[1]*N₁p*N₂p),2*χp[1]*N₁p*N₂p])
	(ϕᵢs[1],ϕᵢᵢs[1]) = real.(roots(p2))

	f_objp!(F,ϕ) = begin
		F[1] = df(exp10(ϕ[1]),χp[1],Np)-df((1-exp10(ϕ[2])),χp[1],Np) 
		F[2] = f(exp10(ϕ[1]),χp[1],Np)-f((1-exp10(ϕ[2])),χp[1],Np)-df(exp10(ϕ[1]),χp[1],Np)*exp10(ϕ[1])+df((1-exp10(ϕ[2])),χp[1],Np)*(1-exp10(ϕ[2]))
	end

	solp = Clapeyron.Solvers.nlsolve(f_objp!,big.([-19.,-19.]), LineSearch(Newton()))

	(ϕᵢp[1],ϕᵢᵢp[1]) = Clapeyron.Solvers.x_sol(solp)

	for i in 2:400
		global solp, df, p2
		f_objp!(F,ϕ) = begin
			F[1] = df(exp10(ϕ[1]),χp[i],Np)-df((1-exp10(ϕ[2])),χp[i],Np) 
			F[2] = f(exp10(ϕ[1]),χp[i],Np)-f((1-exp10(ϕ[2])),χp[i],Np)-df(exp10(ϕ[1]),χp[i],Np)*exp10(ϕ[1])+df((1-exp10(ϕ[2])),χp[i],Np)*(1-exp10(ϕ[2]))
		end
		
		solp = Clapeyron.Solvers.nlsolve(f_objp!,big.([ϕᵢp[i-1],ϕᵢᵢp[i-1]]), LineSearch(Newton()))
		(ϕᵢp[i],ϕᵢᵢp[i]) = Clapeyron.Solvers.x_sol(solp)

		p2 = Polynomial([N₂p,(N₁p-N₂p-2*χp[i]*N₁p*N₂p),2*χp[i]*N₁p*N₂p])
		(ϕᵢs[i],ϕᵢᵢs[i]) = real.(roots(p2))
	end

	pltp = plot(exp10.(ϕᵢp),χp.*√(prod(Np)), label="binodal",xlims=(0,1), ylims=(χc-0.5,χc+1).*√(prod(Np)), ylabel=L"$\chi N$", xlabel=L"$\phi$")
	plot!(ϕᵢs,χp.*√(prod(Np)), label="spinodal",color=myscheme[2], linestyle=:dash)
	plot!(ϕᵢᵢs,χp.*√(prod(Np)), label="",color=myscheme[2], linestyle=:dash)
	plot!(pltp, 1 .-exp10.(ϕᵢᵢp), χp.*√(prod(Np)), color=myscheme[1], label="")
	scatter!(pltp,[√(N₂p)/(√(N₁p)+√(N₂p))],[χc.*√(prod(Np))],markerstrokecolor=myscheme[4], color=:white,label="critical point")
	pltp
end

# ╔═╡ 1c053417-1bdd-4764-8a11-34d266b67667
md"""
## Critical scaling
"""

# ╔═╡ c3caa682-d758-4a3a-9146-8aa1ec402ebb
begin
	Nc = exp10.(LinRange(0,7,14))
	χ_c = @. 1/2*(1/√(1)+1/√(Nc))^2
	ϕ_c = @. √(Nc) / (1+√(Nc))
	pltc = plot(Nc,1 .-ϕ_c, label="",xaxis=:log, yaxis=:log, ylabel=L"$\phi_c$", xlabel=L"$N$", xlims=(1e0,1e6))
	plot!(pltc,[2e4,2e5],[1e-2,exp10(-2.5)],label="slope ½")
	pltc
end

# ╔═╡ e5eba552-67a0-475c-ad11-379b68dde089
begin
	pltc2 = plot(Nc[1:end-1],χ_c[1:end-1].-χ_c[end], label="",xaxis=:log, yaxis=:log, ylabel=L"$\chi_c-\chi_c^\infty$", xlabel=L"$N$", xlims=(1e0,1e6))
	plot!(pltc2,[2e3,2e4],[1e-1,exp10(-1.5)],label="slope ½")
	pltc2
end

# ╔═╡ a33b41cc-6b41-4c37-9e4f-282c51bb1b1e
md"""
## Real behaviour of polymer mixtures
"""

# ╔═╡ a396d363-cc6a-40d2-98b9-e66292599103
@bind N₁r Slider(1:1:100)

# ╔═╡ 35761ebf-471c-4210-a63d-c091f0cd9446
md"""
N₁ = $(N₁r)
"""
...
# ╔═╡ 444f800e-9ef8-4423-8f2c-df6bef8e0d2c
begin
	Nr = [N₁r,N₂r]
	χcr = 1/2*(1/√(N₁r)+1/√(N₂r))^2

	T = LinRange(300,1000,400)
	...
	if any(is_split)
		if maximum(χr) == χr[1]
			id_start = 1
			id_end = findlast(is_split)
			step = 1
		else
			id_start = 400
			id_end = findfirst(is_split)
			step = -1
		end
		
		p3 = Polynomial([N₂r,(N₁r-N₂r-2*χr[id_start]*N₁r*N₂r),2*χr[id_start]*N₁r*N₂r])
		(ϕᵢsr[id_start],ϕᵢᵢsr[id_start]) = real.(roots(p3))
	
		f_objr!(F,ϕ) = begin
			F[1] = df(exp10(ϕ[1]),χr[id_start],Nr)-df((1-exp10(ϕ[2])),χr[id_start],Nr) 
			F[2] = f(exp10(ϕ[1]),χr[id_start],Nr)-f((1-exp10(ϕ[2])),χr[id_start],Nr)-df(exp10(ϕ[1]),χr[id_start],Nr)*exp10(ϕ[1])+df((1-exp10(ϕ[2])),χr[id_start],Nr)*(1-exp10(ϕ[2]))
		end
	
		solr = Clapeyron.Solvers.nlsolve(f_objr!,big.([-19.,-19.]), LineSearch(Newton()))
	
		(ϕᵢr[id_start],ϕᵢᵢr[id_start]) = Clapeyron.Solvers.x_sol(solr)
		
		for i in id_start+step:step:id_end
			global solr, df, p3
			f_objr!(F,ϕ) = begin
				F[1] = df(exp10(ϕ[1]),χr[i],Nr)-df((1-exp10(ϕ[2])),χr[i],Nr) 
				F[2] = f(exp10(ϕ[1]),χr[i],Nr)-f((1-exp10(ϕ[2])),χr[i],Nr)-df(exp10(ϕ[1]),χr[i],Nr)*exp10(ϕ[1])+df((1-exp10(ϕ[2])),χr[i],Nr)*(1-exp10(ϕ[2]))
			end
			
			solr = Clapeyron.Solvers.nlsolve(f_objr!,big.([ϕᵢr[i-step],ϕᵢᵢr[i-step]]), LineSearch(Newton()))
			(ϕᵢr[i],ϕᵢᵢr[i]) = Clapeyron.Solvers.x_sol(solr)
...
manifest_format = "2.0"
project_hash = "21a70b99f6cac2521f0f914cc1443f653537c152"
...
git-tree-sha1 = "bd5ae2e9ce043cdb1882d922dd339760646b888a"
